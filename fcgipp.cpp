...
namespace fcgi {
    ...
    FastCGIClient::FastCGIClient(std::string listen) {        
        ...
        addrlen = sizeof(sun->sun_family) + strlen(sun->sun_path);
    }
     ...
     * @return void
     */
    void FastCGIClient::send(const std::string &stdin) {
        if (_connect()) {
            _send(stdin);
        }
     ...
     * @return string
     */
    std::string FastCGIClient::request(const std::string &stdin) {
        response.erase();
        if (_connect()) {
            _send(stdin);
     ...
     * @return void
     */
    void FastCGIClient::_send(const std::string &stdin) {
        ...
        /* FCGIパラメータの書き込み */
        FCGX_Stream *paramsStream = FCGX_CreateWriter(sockfd, requestId, WRITER_LEN, FCGI_PARAMS);
    ...
    int FastCGIClient::_write(unsigned char type, const char *content, int length) {
        ...
        int count = -1;
        /* ヘッダーの送信 */
        count = write(sockfd, (char *)&header, HEADER_LEN);
        if (count != HEADER_LEN) {
            throw "Unable to write fcgi header.";
        }

        /* コンテンツの送信 */
        count = write(sockfd, content, length);
        if (count != length) {
    ...
    int FastCGIClient::_read() {
        ...
        int count = -1;
        count = read(sockfd, (char *)header, sizeof(header));
        if (count != HEADER_LEN) {
        ...
        if (length > 0) {
            ...
            count = read(sockfd, buf, length + header->paddingLength);
    ...
    bool FastCGIClient::_connect() {
        if ((sockfd = socket(sockdom, SOCK_STREAM, 0)) < 0) {
            log::error("Unable to connect fcgi server.");
            return false;
        }
        
        if (connect(sockfd, address, addrlen) < 0) {
            log::debug("Unable to connect fcgi server.");
            return false;
        }
